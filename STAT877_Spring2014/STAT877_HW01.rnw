...
\hline
Full data&LOD scores are considerately &LOD scores are considerately \\
&different than those in H-K&different than those in EM\\
\hline
Selective genotyping&LOD scores are approximately&LOD scores are approximately\\
&the same as H-K method&the same as EM method\\
\hline 
...
\textbf{Conclusions:}
\begin{itemize}
\item In H-K method, the treatment of missing genotype information is less than ideal. Thus, its approximation of standard (EM) interval mapping can be poor in regions of low genotype information. Such cases are widely spaced or incompletely genotyped markers as in our homework dataset. 
\item In addition, H-K performance is extremely poor in the case of selective genotyping. 
\end{itemize}
