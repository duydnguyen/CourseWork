...
plot(out.em, out.hk, chr=c(1:6),color=c("blue", "red"), ylab="LOD score")
@
...
\end{figure}
\noindent
\textbf{Remark:} From the performed tests, we see a difference in the significant threshold when using the stratified permutation test. The $5\%$ threshold is $5.97$ (versus $2.42$ from the traditional permutation test). 
...
<<Plots EM and H-K together, echo=TRUE>>=
plot(out.em.sub, out.hk.sub, chr=c(1:6),color=c("blue", "red"), ylab="LOD score")
@
...
@
\noindent
\textbf{Remark:}The $5\%$ threshold is $2.49$ (versus $2.25$ from the standard permutation test with full data). The $20\%$ threshold is $1.78$ (versus $2.69$ from the permutation test with full data). Thus, in terms of the same standard permutation test procedure, we see that there is not a significant difference of genome-wide LOD thresholds between full data and data excluding non-geonotyped. individuals.  
